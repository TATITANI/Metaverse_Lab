...
void parseData() {
  ...
  Serial.println(String(pressure[0]) + "," + String(pressure[1]) + "," + String(pressure[2]));
}
...
void SetupMotor() {
  ...
  // 입력 값이 없을 때는 모터의 각도를 90도로 설정(손이 마음대로 움직일 수 있는 상태)
  // for (int i = 0; i < 3; i++) {
  //   currentServoAngles[i] = targetServoAngles[i] = 90;
  // }
}
...
void MotorControl() {
  ...
  potentiometer_value[0] = Clamp(analogRead(A2), 260, 830);  // analogdRead - 0~1023 가변저항(엄지: A2)
  potentiometer_value[1] = Clamp(analogRead(A3), 260, 830);  // analogdRead - 0~1023 가변저항(검지: A3)
  potentiometer_value[2] = Clamp(analogRead(A4), 260, 830);  // analogdRead - 0~1023 가변저항(중지: A4)

  int potentAngle[3];  // 가변저항 나사 각도
  potentAngle[0] = Map(potentiometer_value[0], 830, 260, 0, 90);
  potentAngle[1] = Map(potentiometer_value[1], 260, 830, 0, 90);
  potentAngle[2] = Map(potentiometer_value[2], 260, 830, 0, 90);

  // Serial.println(String(analogRead(A2)) + ", " + String(potentAngle[0]));
  ...
  if (millis() - lastMotorUpdatedTime > delay_value) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      servo[i].write(currentServoAngles[i]);  // 서보에 현재 각도를 반영
      msg += String(currentServoAngles[i]) + ", " + String(targetServoAngles[i]) + " / ";
    }
    Serial.println(msg);
